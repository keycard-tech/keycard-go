...
func TestFindTag(t *testing.T) {
	...
	data := hexutils.MustHexToBytes("C1 02 BB CC C2 04 C3 02 11 22 C3 02 88 99")
	...
	// tag not found
	data = hexutils.MustHexToBytes("C1 00")
	_, err = FindTag(data, Tag{0xC2})
	assert.Equal(t, &ErrTagNotFound{Tag{0xC2}}, err)

	// sub-tag not found
	data = hexutils.MustHexToBytes("C1 02 C2 00")
...
func TestFindTagN(t *testing.T) {
	data := hexutils.MustHexToBytes("0A 01 A1 0A 01 A2")
